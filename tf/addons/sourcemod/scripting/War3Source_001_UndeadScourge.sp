...
float Reincarnation[9]={0.0,60.0,55.0,50.0,45.0, 44.0, 43.0, 42.0, 40.0};
float UnholySpeed[9]={1.0,1.05,1.10,1.15,1.24, 1.26, 1.28, 1.3, 1.32};
float VampirePercent[9]={0.0,0.15,0.2,0.25,0.3, 0.325, 0.35, 0.375, 0.4};
#endif
...
public OnWar3LoadRaceOrItemOrdered2(num,reloadrace_id,String:shortname[])
{
	...
	if(num==RACE_ID_NUMBER||(reloadrace_id>0&&StrEqual(RACE_SHORTNAME,shortname,false)))
	{
		...
		SKILL_LEECH=War3_AddRaceSkill(thisRaceID,"Vampiric Aura","Leech Health\nYou recieve up to 40% of your damage dealt as Health\nCan not buy item mask any level",false,8);
		SKILL_SPEED=War3_AddRaceSkill(thisRaceID,"Unholy Aura","You run up to 32% faster",false,8);
		SKILL_LOWGRAV=War3_AddRaceSkill(thisRaceID,"Blood Lust","When you gain health from Vampiric Aura, your crit chance by +2%.\nCrit Chance resets on death.\nCrits count as 50% damage increase.",false,8);
		SKILL_SUICIDE=War3_AddRaceSkill(thisRaceID,"Reincarnation","When you die, you revive on the spot.\nHas a base 60 second cooldown.\nDecreases cooldown by -5s each upgrade. After 4 upgrades, reduces to -1s.",true,8);
		War3_CreateRaceEnd(thisRaceID);
