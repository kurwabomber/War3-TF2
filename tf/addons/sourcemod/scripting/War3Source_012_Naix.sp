...
new Float:HPPercentHealPerKill[9] = { 0.0, 0.05,  0.10,  0.12,  0.15 , 0.16, 0.17, 0.18, 0.20}; //SKILL_INFEST settings
//Skill 1_1 really has 5 settings, so it's not a mistake
new HPIncrease[9]       = { 0, 10, 20, 25, 35, 40, 45, 50, 55};     //Increases Maximum health

new Float:feastPercent[9] = { 0.0, 0.04,  0.06,  0.08,  0.1, 0.12, 0.135, 0.15, 0.16};   //Feast ratio (leech based on current victim hp


new Float:RageAttackSpeed[9] = {1.0, 1.15,  1.25,  1.3334,  1.45, 1.55, 1.65, 1.75, 1.85};   //Rage Attack Rate
new Float:RageDuration[9] = {0.0, 3.0,  3.5,  4.0,  4.5, 4.75, 5.0, 5.25, 5.5};   //Rage duration
...
public OnWar3LoadRaceOrItemOrdered2(num,reloadrace_id,String:shortname[])
{
	if(num==RACE_ID_NUMBER||(reloadrace_id>0&&StrEqual("naix",shortname,false)))
	{
		...
		SKILL_BLOODBATH = War3_AddRaceSkill(thisRaceID, "Blood Bath","Increases leechable health of the Naix by 10-55,\nmaking all his other skills worthy",false,8);
		SKILL_FEAST = War3_AddRaceSkill(thisRaceID, "Feast","Regenerates 4-16% percent of enemy's current HP chance on hit.",false,8);
		ULT_RAGE = War3_AddRaceSkill(thisRaceID, "Rage","Naix goes into a maddened Rage, gaining 15-85% attack speed for 2-5.5 seconds", true,8);
