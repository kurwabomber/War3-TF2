...
// Combat Experience
new Float:HeadshotDMG[9] = {1.0,1.06,1.12,1.18,1.24,1.265,1.29,1.315,1.34};

// On The Move
new Float:MoveMultiplier[9] = {1.0,1.05,1.1,1.15,1.2,1.225,1.25,1.275,1.3};
new Float:ScopeMoveMultiplier[9] = {1.0,1.04,1.08,1.12,1.16,1.2,1.22,1.24,1.26};

// Adrenaline
new Float:AdrenalineRegen[9]={0.0,0.75,1.5,2.25,3.0,3.25,3.5,3.75,4.0};
new Float:AdrenalineAttackspeed[9]={1.0,1.025,1.05,1.075,1.1,1.1125,1.125,1.13375,1.15};

// UAV Scan
new Float:UAVCooldown[9]={50.0,50.0,46.0,42.0,38.0,36.0,34.0,32.0,30.0};
...
public OnWar3LoadRaceOrItemOrdered2(num,reloadrace_id,String:shortname[])
{
	if(num==RACE_ID_NUMBER||(reloadrace_id>0&&StrEqual("commando",shortname,false)))
	{
		...
		SKILL_COMBAT=War3_AddRaceSkill(thisRaceID,"Combat Experience","All headshots (hits to the head) deal 6 to 34% more damage.",false,8);
		SKILL_MOVE=War3_AddRaceSkill(thisRaceID,"On The Move","Increased movespeed and you move faster while aiming.\nIncreases movepseed by 5 to 30% and scope speed by 4 to 26%.",false,8);
		SKILL_ADRENALINE=War3_AddRaceSkill(thisRaceID,"Adrenaline","You regenerate health & increase attackspeed.\nRegenerates 0.75 to 4 HP per second and increases attackspeed by 2.5 to 15%.",false,8);
		ULT_SCAN=War3_AddRaceSkill(thisRaceID,"UAV Scan","You and teammates gain the ability to see targets anywhere. Lasts 7 seconds.\nCooldown is 50 to 30 seconds.",true,8);
...
GiveCommandoPerks(client)
{
	...
	if(IsValidEntity(weapon) && skill_level > 0)
	{
