...
#if GGAMETYPE == GGAME_TF2
		shopItem[FIREORB]=War3_CreateShopItemT("fireorb","chance to ignite victim", 10, 4000);
#endif

		shopItem[COURAGE]=War3_CreateShopItem("Armor of Courage","courage","+25% dmg reduction","increases up to 15% resistance against all physcial damage\n(does not block magical)\n(does not stack with other armor increases)",10,3000);

		shopItem[FAITH]=War3_CreateShopItem("Armor of Faith","faith","+25% magic dmg reduction","increases up to 15% resistance against all magical damage\n(does not block physical)\n(does not stack with other armor increases)",10,3000);
...
public OnItemPurchase(client,item)
{
	...
	{
		War3_SetBuffItem(client,fArmorMagic,shopItem[FAITH],5.0); //mvm
		War3_NotifyPlayerItemActivated(client,shopItem[FAITH],true);
	}
	else if(item==shopItem[COURAGE]&&ValidPlayer(client))
	{
		War3_SetBuffItem(client,fArmorPhysical,shopItem[COURAGE],5.0); //mvm
...
public OnItemLost(client,item){ //deactivate passives , client may have disconnected
	...
	{
		War3_SetBuffItem(client,fDamageModifier,shopItem[CLAW],0.0);
		War3_NotifyPlayerItemActivated(client,shopItem[CLAW],false);//GetConVarFloat(ClawsAttackCvar)
	}
	else if(item==shopItem[FROST])
	{
	...
	else if(item==shopItem[MASK])
	{
		...
		War3_SetBuffItem(client,fVampirePercent,shopItem[MASK],0.0);
	}
}
