...
new Float:StrDMG[9]={0.0,0.02,0.0225,0.023,0.024,0.025,0.027,0.029,0.03};
//new Float:StrHP[9]={0.0,6.0,6.0,6.0,6.0,6.375,6.75,7.125,7.5};
new Float:StrRegen[9]={0.0,0.5,0.5,0.5,0.5,0.55,0.6,0.65,0.7};
//Agility Morph
int agiPoints[MAXPLAYERSCUSTOM];
//new Float:AgiAttSPD[9]={0.0,0.03,0.03,0.03,0.03,0.03125,0.0325,0.03375,0.035};
new Float:AgiMove[9]={0.0,0.035,0.035,0.035,0.035,0.0365,0.0375,0.03875,0.04};
new Float:AgiDef[9]={0.0,0.5,0.5,0.5,0.5,0.6,0.7,0.8,0.9};
int morphPoints[MAXPLAYERSCUSTOM];
int allocatableMorphPoints[9]={0,1,2,3,4,4,4,4,4};
//Liquid Form
new Float:Evasion[9]={0.0,0.03,0.04,0.05,0.06,0.07,0.075,0.084,0.1};
//Morph
new Float:MorphRange[9]={0.0,675.0,800.0,925.0,1050.0,1100.0,1150.0,1200.0,1250.0};
new Float:MorphCooldown[9]={0.0,80.0,75.0,70.0,60.0,60.0,60.0,60.0,60.0};
new Float:MorphDuration[9]={0.0,6.0,6.0,6.0,6.0,6.25,6.5,6.75,7.0};
...
public OnWar3LoadRaceOrItemOrdered2(num,reloadrace_id,String:shortname[])
{
	if(num==RACE_ID_NUMBER||(reloadrace_id>0&&StrEqual("morph",shortname,false)))
	{
		...
		SKILL_STRMORPH=War3_AddRaceSkill(thisRaceID,"Strength Morph","Gives +1 morph points.\nPer each point assigned to strength, you gain 2% to 3% damage & 0.5 to 0.7 regeneration.",false,8);
		SKILL_AGIMORPH=War3_AddRaceSkill(thisRaceID,"Agility Morph","Gives +1 morph points.\nPer each point assigned to agility, you gain 3.5% to 4% movespeed & 0.5 to 0.9 armor.",false,8);
		SKILL_LIQUID=War3_AddRaceSkill(thisRaceID,"Splishy Splashy (Liquid Form)","Gives 3% to 10% evasion.",false,8);
		ULT_MORPH=War3_AddRaceSkill(thisRaceID,"Morph","Changes your race & class to your target. Targets whoever is within a 675 HU radius cone.\n675 to 1250HU range, 80s to 60s cooldown, and 6 to 7s duration.",true,8);
...
public OnRaceChanged(client,oldrace,newrace)
{
	...
	else
	{
		strPoints[client] = 0;
		agiPoints[client] = 0;
		new strMorphLevel=War3_GetSkillLevel(client,thisRaceID,SKILL_STRMORPH);
		new agiMorphLevel=War3_GetSkillLevel(client,thisRaceID,SKILL_AGIMORPH);
		new maxPoints = 0;
		if(strMorphLevel > 0)
		{
			maxPoints += allocatableMorphPoints[strMorphLevel];
		}
		if(agiMorphLevel > 0)
		{
			maxPoints += allocatableMorphPoints[agiMorphLevel];
		}
		morphPoints[client] = maxPoints;
	}
}
public OnAbilityCommand(client,ability,bool:pressed)
{
	if(War3_GetRace(client)==thisRaceID && pressed && ValidPlayer(client))
	{
		...
		switch(ability)
		{
			case 0:
			{
				if(str_skill_level>0&&!Silenced(client))
				{
			...
			case 2:
			{
				if(agi_skill_level>0&&!Silenced(client))
				{
					if(morphPoints[client] > 0)
					{
						agiPoints[client]++;
