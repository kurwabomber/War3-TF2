...
new Float:VitalityHealed[]={0.0,1.0,2.0,3.0,4.0, 4.5, 5.0, 5.5, 6.0}; // How much HP Vitality heals each second

// Burning Spear stacking effect
new SpearDamage[]={0,1,2,3,4,5,6,7,8}; // How much damage does a stack do?
...
new Float:BerserkerBuffASPD[]={0.0,0.01,0.02,0.03,0.04, 0.045, 0.05, 0.055, 0.06};      // to get the total buff...

// Life Break costs / damage dealt
new Float:LifeBreakHPVictim[]={0.0,0.20,0.30,0.40,0.50, 0.525, 0.55, 0.575, 0.6}; // Percentage of how much HP the caster loses
new Float:LifeBreakHPCaster[]={0.0,0.10,0.15,0.20,0.25, 0.225, 0.2, 0.175, 0.175};    // Percentage of how much HP the victim loses
...
public OnWar3LoadRaceOrItemOrdered2(num,reloadrace_id,String:shortname[])
{
	if(num==RACE_ID_NUMBER||(reloadrace_id>0&&StrEqual("sacredw",shortname,false)))
	{
		...
		SKILL_VITALITY=War3_AddRaceSkill(thisRaceID,"Inner Vitality","Passively recover 1 to 6HP per second.\nWhen below 40% you heal twice as fast.",false,8);
		SKILL_SPEAR=War3_AddRaceSkill(thisRaceID,"Burning Spear","(+ability) Passively lose 5% maxHP, but set enemies ablaze.\nDeals 1 to 8 DPS for next 3 seconds.\nStacks 5 times.",false,8);
		SKILL_BLOOD=War3_AddRaceSkill(thisRaceID,"Berserkers Blood","Gain 1 to 6 percent attack speed for each 7 percent of your health missing",false,8);
		ULT_BREAK=War3_AddRaceSkill(thisRaceID,"Life Break","(+ultimate) Damage yourself (10 to 25 to 17.5% of maxHP) to deal\na great amount of damage (20 to 60% of victim's maxHP)",true,8);
