...
float RevivalChancesArr[]={0.00,0.2,0.3,0.4,0.5,0.55,0.6,0.65,0.7};
...
float BanishChancesArr[]={0.00,0.05,0.10,0.15,0.20,0.25,0.30,0.325,0.35};

//for TF only:
float CreditStealChanceTF[]={0.00,0.02,0.04,0.06,0.08,0.10,0.12,0.13,0.14};   //what are the chances of stealing
...
float UltimateMaxDistance[]={0.0,500.0,600.0,700.0,800.0,900.0,1000.0,1100.0,1200.0}; //max distance u can target your ultimate
int UltimateDamageDuration[]={0,4,6,8,10,12,14,15,16}; ///how many times damage is taken (like pyro's fire)
...
public OnWar3LoadRaceOrItemOrdered2(num,reloadrace_id,String:shortname[])
{
	if(num==RACE_ID_NUMBER||(reloadrace_id>0&&StrEqual("mage",shortname,false)))
	{
		...
		SKILL_REVIVE=War3_AddRaceSkill(thisRaceID,"Phoenix","20-70% chance to revive your teammates that die.\nEach time you revive, chance is reduced by half\nto a minimum of 2-16%",false,8); //was 6
		SKILL_BANISH=War3_AddRaceSkill(thisRaceID,"Banish","5-35% of making enemy blind and disoriented for 0.2 seconds",false,8); //was 6
		SKILL_MONEYSTEAL=War3_AddRaceSkill(thisRaceID,"Siphon Mana","2-14% chance of stealing gold based on victim's level via damage",false,8); //was 6
		ULT_FLAMESTRIKE=War3_AddRaceSkill(thisRaceID,"Flame Strike","Burn the enemy over time for 10 damage 4-16 times.\n500-1400 HU range.",true,8); //was 6
