...
new Float:immunityChance[9]={0.0,0.2,0.4,0.6,0.8,0.85,0.9,0.95,1.0};
new Float:DefenseBonus[9]={0.0,0.5,1.0,1.5,2.0,2.25,2.5,2.75,3.0};
new Float:Reincarnation[9]={0.0,60.0,55.0,50.0,45.0, 44.0, 43.0, 42.0, 40.0};
new Float:FlameBaseDMG[9]={0.0,5.0,10.0,15.0,20.0, 22.0, 24.0, 26.0, 28.0};
...
public OnWar3LoadRaceOrItemOrdered2(num,reloadrace_id,String:shortname[])
{
	if(num==RACE_ID_NUMBER||(reloadrace_id>0&&StrEqual("phoenix",shortname,false)))
	{
		...
		SKILL_SPELLIMMUNITY=War3_AddRaceSkill(thisRaceID,"Spell Immunity","Grants partial immunity to all effects.\nEach level increases chance per second. 20% to 100%.",false,8);
		SKILL_RESISTANT=War3_AddRaceSkill(thisRaceID,"Resistant Skin","Gives a boost to defense. Gives 0.5 to 3 physical armor.",false,8);
		SKILL_REVIVE=War3_AddRaceSkill(thisRaceID,"Revival","Self-revives with a cooldown. Cooldownn is 60 to 40 seconds.",false,8);
		ULT_FIRE=War3_AddRaceSkill(thisRaceID,"Phoenix Fire","Ignites everyone in a 600 HU radius. Deals 5 to 28 damage. Cooldown is 1.5 seconds.",true,8);
		War3_CreateRaceEnd(thisRaceID);
		War3_AddSkillBuff(thisRaceID, SKILL_RESISTANT, fArmorPhysical, DefenseBonus);
		W3SkillCooldownOnSpawn(thisRaceID,SKILL_REVIVE,30.0,true);
	}
}
