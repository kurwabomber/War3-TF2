...
new MaximumWards[9]={0,1,2,3,4,4,5,5,6};
new PushPower[9]={0,2,3,4,5,6,7,8,8};

new Float:SwimSpeed[9]={0.0,1.5,1.75,2.0,2.25,2.5,2.65,2.8,3.0};
		...
		if(ValidPlayer(i,true))
		{
			...
			if(War3_GetRace(i)==thisRaceID)
			{
				if(GetEntityFlags(i) & FL_INWATER)
				{
					...
					if(skill_level>0 && fWaterBreathing[i]==0.0)
					{
						//DP("%i = new",i);
					...
					if(skill_level>0 && fWaterBreathing[i]<(GetGameTime()-3.0))
					{
						//DP("%i = waterbreathing = reset",i);
						bWaterBreathing[i]=false;
						fWaterBreathing[i]=GetGameTime();
					}
					if(skill_level>0 && fWaterBreathing[i]<(GetGameTime()-2.0))
					{
						//DP("%i = waterbreathing = false",i);
						bWaterBreathing[i]=false;
						continue;
					}
					if(skill_level>0 && fWaterBreathing[i]<(GetGameTime()-1.0))
					{
...
public void OnAbilityCommand(int client, int ability, bool pressed, bool bypass)
{
	...
	if(War3_GetRace(client)==thisRaceID && ability==0 && pressed && IsPlayerAlive(client))
	{
		...
		if(skill_level>0&&!Silenced(client))
		{
			if(War3_GetWardCount(client)<MaximumWards[skill_level])
			{
					...
					if(War3_CreateWardMod(client, location, 120, 120.0, 0.5, "zap", SKILL_BUGZAP_WARD, PushPower, WARD_TARGET_ENEMYS)>-1)
					{
	...
	if(War3_GetRace(client)==thisRaceID && ability==2 && pressed && IsPlayerAlive(client))
	{
		...
		if(skill_level>0&&!Silenced(client)&&SkillAvailable(client,thisRaceID,SKILL_HYPERSWIM,true))
		{
...
public void OnUltimateCommand(int client, int race, bool pressed, bool bypass)
{
	if(race==thisRaceID && pressed && ValidPlayer(client,true,true) && !Silenced(client))
	{
		...
		if(ult_level>0 && War3_SkillNotInCooldown(client,thisRaceID,ULTIMATE,true))
		{
